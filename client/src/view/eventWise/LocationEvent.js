...
export default function LocationEvent(){

    const auth=useContext(authContext)
    // console.log(auth.token)
    const [values,setValues]=useState({eve_place:""})
    const[click,setClick]=useState(false)
    const [result,setResult]=useState([])
    
    async  function handle(e){
        e.preventDefault()
        const url=`http://localhost:2050/api/event/eve_placeview/${values.eve_place}`
        const headers = {
    ...
    return(
        ...
        <form>
            <div>
                ...
                <input type="text" placeholder="Enter To Location" 
